...
import { deleteListingPhotos } from '../../services/photoStorage';
...
export function ListingsManager({ onEdit, onAdd }: ListingsManagerProps) {
  const { listings, loading, error, refetch, setListingStatus, deleteListing } = useListings();
  ...
  const handleStatusChange = async (listing: Listing, status: ListingStatus) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : '變更狀態失敗');
    } finally {
      setBusyId(null);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // 物件本身刪除成功後才清照片；照片清不掉只記錄警告，不影響刪除結果。
      void deleteListingPhotos(target.photos);
    } catch (err) {
      setActionError(err instanceof Error ? err.message : '刪除物件失敗');
    } finally {
      setBusyId(null);
    }
  };
    ...
    <div className="px-5 py-5">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-ink-500">{loading ? '載入中…' : `共 ${listings.length} 筆物件`}</p>
        </div>
        <button
          type="button"
          onClick={onAdd}
      ...
      {error ? (
        ...
        <>
          {actionError && (
            <div className="mb-3 rounded-lg border border-danger-600/30 bg-danger-bg px-3 py-2 text-xs font-medium text-danger-600">
              {actionError}
            </div>
          )}
                ...
                <div key={listing.id} className="overflow-hidden rounded-xl border border-border bg-white">
                  <div className="flex gap-3 p-3">
                    <img
                      ...
                      alt=""
                      className="h-20 w-24 shrink-0 rounded-lg bg-surface-alt object-cover"
                    />
                  ...
                  <div className="flex items-center gap-2 border-t border-border px-3 py-2">
                    ...
                    <select
                      ...
                      className={`min-h-[44px] flex-1 rounded-lg border px-2 text-xs font-bold outline-none disabled:opacity-50 ${statusSelectStyle[listing.status]}`}
                    >
